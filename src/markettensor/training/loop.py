...
def _train_torch_model(
    ...
    loader = DataLoader(
        SequenceDataset(train_arrays.features, train_arrays.targets),
        batch_size=int(config["model"]["batch_size"]),
        shuffle=False,
    )

    model.train()
    for _ in range(int(config["model"]["epochs"])):
        epoch_loss = 0.0
    ...
    model.eval()
    with torch.no_grad():
        logits = model(torch.from_numpy(test_arrays.features)).numpy()
    probability = _sigmoid(logits)
    prediction = (probability >= 0.5).astype(int)
    target_rows = scaled_test.groupby("symbol", observed=True).cumcount() >= lookback - 1
    test_targets = scaled_test.loc[
        target_rows, ["timestamp", "symbol", "target", "future_return"]
    ].reset_index(drop=True)
    _save_model(model, run_dir / "model.pt")
    save_json({"loss_history": tracker.losses}, run_dir / "training_history.json")
